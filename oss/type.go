...
type ListObjectVersionsResult struct {
	...
	CommonPrefixes      []string                       `xml:"CommonPrefixes>Prefix"` // You can think of commonprefixes as "folders" whose names end with the delimiter
	ObjectDeleteMarker  []ObjectDeleteMarkerProperties `xml:"DeleteMarker"`          // DeleteMarker list
	ObjectVersions      []ObjectVersionsProperties     `xml:"version"`               // version list
}
...
type ObjectVersionsProperties struct {
...
func decodeListObjectVersionsResult(result *ListObjectVersionsResult) error {
	...
	// decode deleteMarker
	for i := 0; i < len(result.ObjectDeleteMarker); i++ {
		result.ObjectDeleteMarker[i].Key, err = url.QueryUnescape(result.ObjectDeleteMarker[i].Key)
		if err != nil {
			return err
		}
	}
